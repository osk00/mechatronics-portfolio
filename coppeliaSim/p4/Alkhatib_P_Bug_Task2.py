...
def generate_sensor_directions(starting_angle, nbr_sensors):
    direction_vectors = []
    angle_step = 360.0 / nbr_sensors # 30
    for i in range(nbr_sensors):
        angle_deg = starting_angle + i * angle_step
        angle_rad = math.radians(angle_deg)
        x = math.cos(angle_rad)
        y = math.sin(angle_rad)
        direction_vectors.append([x, y, 0]) #v_i = {[x1, y1, 0], [x2, y2, 0], [xi, yi, 0],..}
...
# sensor pointing towards starting angle
# returns all directions vectors with unit length
def generate_obstacle_avoidance_vector(detected_distances):
    avoidance_vector = np.array([0., 0., 0.])
    d0 = 0.2  # 20 cm influence radius
    K = 0.0005 # gain to scale the repulsion
    direction_vectors = generate_sensor_directions(-90, 12)

    for sensor_index, dist in detected_distances:
        if dist < 0.001:
            continue  # ignore too-close errors
        if dist < d0:
            Force = K * ((1.0 / dist) - (1.0 / d0)) / (dist ** 2)
            repulse = -Force * np.array(direction_vectors[sensor_index])
            avoidance_vector += repulse
    return avoidance_vector.tolist()
...
if __name__ == '__main__':
    ...
    while True:
        ...
        avoidance_vector = generate_obstacle_avoidance_vector(detected_distances)
        print("avoidance_vector: ", avoidance_vector, direction_goal)

        # set Bug robot to current position cur_pos
        cur_pos[0] = cur_pos[0] + (direction_goal[0] + avoidance_vector[0]) * 0.01  # 5 mm per iteration
        cur_pos[1] = cur_pos[1] + (direction_goal[1] + avoidance_vector[1]) * 0.01  # 5 mm per iteration
